...
public class StageJukeboxConfigScreen extends Screen {
	...
	private String resolveDiscTitle(ItemStack stack) {
		...
		if (stack.getItem() instanceof RecordItem recordItem) {
			return recordItem.getDisplayName().getString();
		}
		CompoundTag tag = stack.getTag();
		if (tag != null && tag.contains("custom_track")) {
			CompoundTag ct = tag.getCompound("custom_track");
			String title = ct.contains("title") ? ct.getString("title") : (ct.contains("name") ? ct.getString("name") : null);
			String artist = ct.contains("artist") ? ct.getString("artist") : (ct.contains("author") ? ct.getString("author") : null);
