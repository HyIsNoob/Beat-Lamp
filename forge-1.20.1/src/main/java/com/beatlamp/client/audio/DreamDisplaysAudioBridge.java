	...
	public static boolean isAnyDisplayPlayingNear(Vec3 position) {
		...
		for (ActiveDisplay display : ACTIVE_DISPLAYS.values()) {
			...
			if (screen != null) {
				...
				if (getDistanceToScreen(screen, pos) < maxRadius) {
					return true;
				}
			} else {
				return true;
	...
	public static boolean isDisplayPlayingNear(Vec3 position, BlockPos source) {
		...
		for (ActiveDisplay display : ACTIVE_DISPLAYS.values()) {
			...
			if (screen != null) {
				if (!checkDimension(screen)) continue;
				if (source != null) {
					if (matchesSource(screen, source)) {
						return true;
					}
				} else if (getDistanceToScreen(screen, pos) < maxRadius) {
					return true;
				}
			} else {
	...
	public static float getLevelAt(Vec3 position, BlockPos source) {
		...
		for (ActiveDisplay display : ACTIVE_DISPLAYS.values()) {
			...
			if (screen != null) {
				...
				falloff = (source != null && matchesSource(screen, source)) ? 1.0F : falloff(dist);
