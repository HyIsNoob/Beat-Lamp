...
public class BeatLampClient {
	...
	public static void tickStageLight(StageLightBlockEntity light) {
		...
		if (light.getMode() == StageLightMode.BEAT_STEP) {
			long gameTime = level.getGameTime();
			if (light.beamBeat > 0.35F && gameTime - light.lastBeatChange >= 6L) {
	...
	public static void tickFogGenerator(FogGeneratorBlockEntity fog) {
		...
		for (int i = 0; i < count; i++) {
			...
			if (isColored) {
				addStageParticle(level, new net.minecraft.core.particles.DustParticleOptions(new org.joml.Vector3f(r, g, b), 2.2F), px, py, pz, vx * 0.9, vy, vz * 0.9);
				if (i % 3 == 0) {
	...
	public static void init() {
		...
		RainbowLedBlockEntity.controllerUser = led -> Minecraft.getInstance().setScreen(new RainbowLedConfigScreen(led));
	}
}
