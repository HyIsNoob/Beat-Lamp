...
public class LaserProjectorConfigScreen extends Screen {
	...
	private ValueSlider beamCountSlider;
	...
	protected void init() {
		...
		this.beamCountSlider = this.addRenderableWidget(
			new ValueSlider(centerX - 100, y + 48, Component.translatable("screen.beatlamp.laser.beam_count"), this.beamCount, 1, 9, "%d") {
				@Override
				protected void applyValue() {
					LaserProjectorConfigScreen.this.beamCount = (int) Math.round(Mth.lerp(this.value, 1, 9));
				}
			}
		);

		this.spreadSlider = this.addRenderableWidget(
			new ValueSlider(centerX + 2, y + 48, Component.translatable("screen.beatlamp.laser.spread"), this.spread, 10.0, 90.0, "%.0f°") {
				@Override
				protected void applyValue() {
					LaserProjectorConfigScreen.this.spread = (float) Mth.lerp(this.value, 10.0, 90.0);
				}
			}
		);

		this.speedSlider = this.addRenderableWidget(
			new ValueSlider(centerX - 100, y + 72, Component.translatable("screen.beatlamp.speed"), this.speed, 0.25, 4.0, "%.2fx") {
				@Override
				protected void applyValue() {
					LaserProjectorConfigScreen.this.speed = (float) Mth.lerp(this.value, 0.25, 4.0);
				}
			}
		);
	...
	public void onClose() {
		...
		PlatformNetwork.sendToServer(
			new LaserProjectorConfigurePayload(
				this.pos, this.mode, this.beamCount, this.spread, this.speed, this.color, this.unlink, this.dmxEnrolled, finalName
			)
		);
		super.onClose();
	}
